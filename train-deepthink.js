...
async function main() {
    console.log('\n╔════════════════════════════════╗');
    console.log('║  ALYA-B20-DEEPTHINK TRAINER   ║');
    console.log('╚════════════════════════════════╝\n');
    ['models', 'data'].forEach(d => { if (!fs.existsSync(d)) fs.mkdirSync(d); });
    const lines = fs.readFileSync('./data/deepthink.jsonl', 'utf8').split('\n').filter(l => l.trim());
    const trainingData = lines.map(l => { try { return JSON.parse(l); } catch(e) { return l; } });
    console.log(`[+] ${trainingData.length} samples\n`);
    const model = new AlyaB20DeepThink();
    await model.train(trainingData, 3);
    model.save('./models/alya-b20-deepthink.json');
    console.log('[+] Run: node chat-deepthink.js\n');
}
